...
class Repository extends CI_Controller {
		...
		public function __construct()
    {
			...
			$this->load->model('mahasiswa/repository_model','dbObject');
    ...
    public function index()
    {
      ...
      $this->load->view('mahasiswa/repository/index',$data);
      $this->load->view('mahasiswa/templates/footer');
		...
		public function all()
		{
			...
			$this->load->view('mahasiswa/templates/navbar');
			$this->load->view('mahasiswa/templates/sidebar');
			$this->load->view('mahasiswa/repository/all',$data);
			$this->load->view('mahasiswa/templates/footer');
		}
		...
		public function create($param1='')
    {
			if($param1=='do')
			{
				...
				if (!is_dir('assets/repository/mahasiswa/'.$category_file_name)){
						$oldmask = umask(0);
						mkdir('./assets/repository/mahasiswa/' . $category_file_name, 0777, TRUE);
						umask($oldmask);
				}
						$file_config['upload_path']          = 'assets/repository/mahasiswa/'.$category_file_name;
			...
			//-------------------------- End of Upload File Gambar Repository -----------------------//
						...
						if ($upload_file_gbr == FALSE && $upload_file == FALSE)
						{
							...
							$this->session->set_flashdata('notif','<div class="alert alert-success"> Repository Telah berhasil dibuat dan masuk dalam antrian persetujuan </div>');
							redirect('mahasiswa/repository/log_acceptance');
						}
			}else{
			$this->session->set_userdata('submenu','repository');
      $data['title']='Repository Fasilkom-TI';
      ...
      $this->load->view('mahasiswa/templates/navbar');
      $this->load->view('mahasiswa/templates/sidebar');
      $this->load->view('mahasiswa/repository/create');
      $this->load->view('mahasiswa/templates/footer');
			}
    }
		...
		public function edit($param1='',$param2='')
    {
      ...
      $this->load->view('mahasiswa/templates/navbar');
      $this->load->view('mahasiswa/templates/sidebar');
      $this->load->view('mahasiswa/repository/edit',$data);
      $this->load->view('mahasiswa/templates/footer');
    }
		...
		public function not_accepted()
		{
			...
			$this->load->view('mahasiswa/templates/navbar');
			$this->load->view('mahasiswa/templates/sidebar');
			$this->load->view('mahasiswa/repository/not_accepted',$data);
			$this->load->view('mahasiswa/templates/footer');
		}
		...
		public function log_acceptance($param1='',$param2='')
		{
			...
			$this->load->view('mahasiswa/templates/navbar');
			$this->load->view('mahasiswa/templates/sidebar');
			$this->load->view('mahasiswa/repository/log_acceptance',$data);
			$this->load->view('mahasiswa/templates/footer');
		}
		...
		public function detail($id)
		{
			...
			$this->load->view('mahasiswa/templates/navbar');
			$this->load->view('mahasiswa/templates/sidebar');
			$this->load->view('mahasiswa/repository/detail',$data);
			$this->load->view('mahasiswa/templates/footer');
		}
		...
		public function create_category($param1='')
		{
				...
				if($param1!=''){
					redirect('mahasiswa/repository/edit/'.$param1);
				}else {
					redirect('mahasiswa/repository/create');
				}
		}
